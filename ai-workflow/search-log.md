...
Rows 1–10 fix every analysis A cut before the real data is used. Rows 12–16 are exploratory: they generate hypotheses, not findings, and nothing from them is published without a Changes entry. Their code is `analysis/explore/a_timing_diagnostic.py`, committed on the owner's instruction. Its results matched the reported run exactly when rerun to a path outside the repository. Its outputs are never committed: the script refuses in-repo paths, `.gitignore` excludes everything but Python source in that folder, and `test_explore_outputs.py` enforces it. Each real-data run is logged as a new row that references these numbers. A is descriptive, and it tests no hypothesis.
